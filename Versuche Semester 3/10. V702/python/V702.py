...
theoR=4.34*60
print(f'\nHalbwertszeit Rhodium 104i= {TR:.5f} s \tTheoriewert: {(theoR)} s  \trelative Abweichung: {((theoR-TR)/theoR)*100} %  \n')
...
theoR2=42.3
print(f'\nHalbwertszeit Rhodium 104= {TR2:.5f} s\tTheoriewert: {(theoR2)} s  \trelative Abweichung: {((theoR2-TR2)/theoR2)*100} % \n')
...
plt.plot(tr,np.log(huell(tr,*params2)),label=r"Fit-Funktion für 104i Rh")
plt.plot(tr[0:(x+y)],np.log(huell(tr[0:(x+y)],*params3)),'m',label=r"Fit-Funktion für 104 Rh")
...
plt.plot(tr[0:(x)],np.log(huell(tr[0:(x)],*params3)),'m',label=r"Fit-Funktion für 104 Rh")
plt.ylabel(r"$ln(N_V-N_0)/[1/15s]$")
plt.xlabel(r"$t/[s]$")
plt.tight_layout()
plt.legend()
